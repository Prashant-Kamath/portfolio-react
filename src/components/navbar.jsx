...
const NavItem = ({ icon: Icon, label, isActive, onClick, mouseX }) => {
	...
	// Button Size: Base 48px -> Peak 76px
	const widthSync = useTransform(distance, [-150, 0, 150], [48, 76, 48]);
	const width = useSpring(widthSync, { mass: 0.1, stiffness: 150, damping: 12 });

	// Icon Size: Base 20px -> Peak 32px (adjust these numbers to taste)
	...
	return (
		<div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50">
			<motion.div
				...
				className="flex items-center gap-3 rounded-[40px] bg-white/90 p-3 shadow-2xl backdrop-blur-md border border-gray-100 h-[90px] w-max"
			>
				{/* Logo Section */}
